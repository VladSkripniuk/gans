...
for protein in ['Alp14', 'Arp3', 'Cki2', 'Mkh1', 'Sid2', 'Tea1']:
	opt.cuda = True
	opt.path = 'LIN_wgan{}/'.format(protein)
	opt.num_iter = 20000
	opt.batch_size = 64
	...
	data = datasets.LINDataset(proteins=[protein], transform=transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)))
	# print(data.path)
	# print(data[0].max())
	# print(data[0].min())
	# dfsdf
	...
	netD = mnistnet.LINnet_D(nc=2, ndf=64, BN=False)
	...
	log = Logger(base_dir=opt.path, tag='LIN{}'.format(protein))
