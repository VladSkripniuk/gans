...
class LINnet_D(nn.Module):
    def __init__(self,nc=1,ndf=64,BN=True,bias=False,n_classes=10): # 128 ok
        ...
        self.layer3 = nn.Sequential(SNConv2d(ndf*2,ndf*4,kernel_size=4,stride=2,padding=1,bias=bias),
                                 ...
                                 nn.LeakyReLU(0.2,inplace=True))
        # 6 x 10
        self.layer4 = nn.Sequential(SNConv2d(ndf*4,ndf*8,kernel_size=4,stride=2,padding=1,bias=bias),
                                 nn.BatchNorm2d(ndf*8),
                                 nn.LeakyReLU(0.2,inplace=True))
        # 3 x 5
        # self.layer5 = nn.Sequential(SNConv2d(ndf*8,1,kernel_size=(3, 5),stride=1,padding=0,bias=bias))#,
                                 # nn.Sigmoid())
    ...
    def forward(self,x):
        ...
        _, y = torch.max(x[:,1:,:,:].std(dim=2).sum(dim=2), dim=1)
...
data = multichannel_LIN(proteins=proteins, transform=transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)))
...
netD = LINnet_D(nc=42,ndf=64,n_classes=41)
